...
Int_t readin(Int_t nrun, Int_t nring, Int_t select)
{
    ...
    for (Int_t i = 0; i < gNRing; i++) {
        ...
        if (fPhaseRing > 3)
            gErrorRing[i] |= (0x2 << (4 * select));

        fLastSeedRing = gSeedRing_rep[i];
    }

    // last pattern may have some problem, correct it
    if ((gNRing > 1) && (fPhaseRing < 3))
        gErrorRing[gNRing - 1] |= (0x2 << (4 * select));
...
Int_t check(Int_t index, Int_t threshold, Int_t select)
{
    ...
    while (l < gNRing) {
        if (gDataRing[index][l] < threshold) { // check whether beam is off
            ...
            for (Int_t i = 0; (i < CUTOFF1) && (m >= 0); i++, m--)
            ...
            for (Int_t i = 0; (i < CUTOFF2) && (p < gNRing); i++, p++)
                gErrorRing[p] |= ((0x4 << (4 * select)) + (0x1000 * select));
        ...
        if (gSeed_rep[l] != 0) {
            ...
            if (pRing < gNRing) {
                if (gErrorRing[pRing] != 0)
                    gError[l] |= (gErrorRing[pRing] | (0x1000 * select));
            } else {
                pRing = 0;
...
Int_t align(Int_t nring, Int_t select)
{
    ...
    while ((pStartRing == 0) && (pStart < gN)) {
        while (((gSeed_rep[pStart] == gSeed_rep[l]) || ((gError[pStart] & ((0xF << (4 * select)) + (0x1000 * select) + 0xF)) != 0)) && (pStart < gN))
            pStart++;

        Int_t m = 0;

        while ((gSeedRing_rep[m] != gSeed_rep[pStart]) && (m < gNRing))
            m++;
    ...
    for (Int_t i = 0; i < pStart; i++)
        gError[i] |= (0x1000 * select);

    for (Int_t i = 0; i < pStartRing; i++)
        gErrorRing[i] |= (0x1000 * select);
    ...
    while (pRing < gNRing) {
        if (gErrorRing[pRing] == 0) {
            ...
            while ((gSeedRing_rep[pRing] == gSeedRing_rep[pLastRing]) && (pRing < gNRing))
                pRing++;
                ...
                for (Int_t i = pLastRing; i < pRing; i++) {
                    if (gHelRing_act[i] == 1) {
                        for (Int_t j = 0; j < nring; j++)
                            fDataP[j] += gDataRing[j][i];
                    } else if (gHelRing_act[i] == -1) {
                        for (Int_t j = 0; j < nring; j++)
                            fDataN[j] += gDataRing[j][i];
                    }
                }
